...
for ms_name in mouse_ctrl_list.union({'GluReaCh42'}):
    ...
    phase_plotphase_plot = dfX_fitT['projected_phase_at_stim']
    Delta_phase = dfX_fitT['br_phase_at_50ms']-dfX_fitT['projected_phase_at_stim']
    Delta_phase[Delta_phase<0] = Delta_phase+2*np.pi
    
    phase_plot = dfX_fitT[['projected_phase_at_stim','br_phase_at_stim']]
    phase_plot = phase_plot.sort_values(by = 'projected_phase_at_stim')
    phase_plot['br_phase_at_stim_unwrap'] = np.unwrap(phase_plot['br_phase_at_stim'])#,discont = 2*np.pi)
    
    ax1.scatter(phase_plot['projected_phase_at_stim'],phase_plot['br_phase_at_stim'],color = col,s = 3)#,color = col)
