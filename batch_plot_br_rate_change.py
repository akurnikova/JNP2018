...
fig, ax_new = plt.subplots(1,1, sharex=False)
...
for ms_name in mouse_list:
    ...
    plt.plot(dfXfilt['br_rate_at_N50ms'],dfXfilt['br_rate_at_100ms'],color = col,marker = '.', linestyle  = '')
    MN1 = np.mean(dfXfilt['br_rate_at_N50ms'])
    MN2 = np.mean(dfXfilt['br_rate_at_100ms'])
    ERR1 = np.std(dfXfilt['br_rate_at_N50ms'])/np.sqrt(dfXfilt['br_rate_at_N50ms'].shape[0])
    ERR2 = np.std(dfXfilt['br_rate_at_100ms'])/np.sqrt(dfXfilt['br_rate_at_100ms'].shape[0])
  #  plt.scatter(MN1,MN2,color = col,s = 30, marker = '>')
    ...
    offset = 1.5
    x = dfXfilt['br_rate_at_N50ms']-offset
    y = dfXfilt['br_rate_at_100ms']-offset
    ## Plot line fit
    a=x.dot(y)/x.dot(x)
    x_plot = np.arange(0,8,0.1)
    plt.plot(x_plot+offset, a*x_plot +offset,color = col)

    
plt.axis('square')
ax_new.set_xlim([1,11])
ax_new.set_ylim([1,11])

fig_title = '/home/asya/Documents/data/mouse_nose/breathing_rate_change.pdf'
